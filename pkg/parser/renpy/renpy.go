...
// Inject generates Ren'Py native translation files: game/tl/<Lang>/nst_translations.rpy and game/nst_language.rpy
func (p *Parser) Inject(ctx context.Context, sourceDir, outputDir string, entries []model.TextEntry) error {
	...
	// 1. Write game/nst_language.rpy (auto-switches to translated language on startup)
	bootScript := fmt.Sprintf("# Generated by NST\ninit -1700 python:\n    config.language = \"%s\"\n    config.default_language = \"%s\"\n", lang, lang)
	_ = os.WriteFile(filepath.Join(gameDir, "nst_language.rpy"), []byte(bootScript), 0644)

	// 2. Write game/tl/<Lang>/nst_translations.rpy
	var sb strings.Builder
	sb.WriteString(fmt.Sprintf("# Generated by NST\ntranslate %s strings:\n\n", lang))
	...
	rpyPath := filepath.Join(tlDir, "nst_translations.rpy")
